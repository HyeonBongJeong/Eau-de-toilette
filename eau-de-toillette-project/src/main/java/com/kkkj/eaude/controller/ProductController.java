...
public class ProductController {
	...
	public ModelAndView insertProduct(ModelAndView mv, MultipartFile file,
			...
			@RequestParam(name = "p_intro") String p_intro, @RequestParam(name = "p_price") int p_price,
		...
		vo.setP_size(p_size);
		vo.setP_img(File.separator + "uploadimage" + File.separator + "imgUpload" + File.separator + "s"
	...
	@ResponseBody
	public String reviewWrite(@RequestParam(name = "r_content") String r_content,
			@RequestParam("file1") MultipartFile file, @RequestParam(name = "r_title") String r_title,
			@RequestParam(name = "p_id") int r_ref, MultipartHttpServletRequest request, HttpSession session,@RequestParam(name="m_id")String m_id) {
		String imgUploadPath = uploadPath + File.separator + "imgUpload";
		String fileName = null;
		...
		if (file != null) {
			try {
				fileName = UploadFileUtils.fileUpload(imgUploadPath, file.getOriginalFilename(), file.getBytes(), path);
				vo.setR_img(File.separator + "uploadimage" + File.separator + "imgUpload" + File.separator + "s"
						+ File.separator + "s_" + fileName);
			} catch (IOException e) {
				e.printStackTrace();
			} catch (Exception e) {
	...
	@ResponseBody
	public Object reviewlist(@RequestParam(name = "page", required = false, defaultValue = "1") int page,
			@RequestParam(name = "p_id") int r_ref) {
		int listCount = 0;
		int startPage = 0;
		int endPage = 0;
		int currentPage = page;
	...
	@ResponseBody
	public Object commentwrite(@RequestParam(name="r_ref")int r_ref,@RequestParam(name="r_content")String r_content,@RequestParam(name="p_id")int p_id,@RequestParam(name="m_id")String m_id) {
	...
	@ResponseBody
	public void productBuy(@RequestParam(name="m_id")String id,@RequestParam(name="p_id")int p_id,@RequestParam(name="ph_count")int ph_count,@RequestParam(name="price")int price) {
	...
	public ModelAndView updateproductpage(@RequestParam(name = "p_id") int p_id, ModelAndView mv) {
		mv.addObject("list", pService.selectProductDetail(p_id));
		mv.setViewName("product_update");
		return mv;
	}

	@RequestMapping(value = "updateproduct.do")
	public ModelAndView updateproduct(ModelAndView mv, MultipartFile file,
			@RequestParam(name = "p_title") String p_title, @RequestParam(name = "p_type") String p_type,
			@RequestParam(name = "p_content") String p_content, @RequestParam(name = "p_size") int p_size,
			@RequestParam(name = "p_intro") String p_intro, @RequestParam(name = "p_price") int p_price,
			MultipartHttpServletRequest request, HttpSession session, @RequestParam(name = "p_id") int p_id) {
		String imgUploadPath = uploadPath + File.separator + "imgUpload";
		String fileName = null;
		String path = request.getSession().getServletContext().getRealPath("/resources/uploadimage");
		...
		vo.setP_img(File.separator + "uploadimage" + File.separator + "imgUpload" + File.separator + "s"
				+ File.separator + "s_" + fileName);
		pService.updateProduct(vo);
		mv.setViewName("redirect:productlist.do");
		return mv;
	}

	@RequestMapping(value = "deleteProduct.do")
	public ModelAndView deleteProduct(ModelAndView mv, @RequestParam(name = "p_id") int p_id) {
